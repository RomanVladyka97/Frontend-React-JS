...
export function RenderMovies(){
    ...
    <main class="main">
        <div class="container main__container" id="main-container">
            <div class="main__title" id="main-title">
                <h2></h2>
                <h2 class="main__serch-result" id="result-caunt"></h2>
            </div>
            <div id="content" class="main__content">
            </div>
            <div id="loading-procces" class="main__loading">
                <span hidden >Завантаження</span>
                <div hidden class="animation__dots dot1"></div>
                <div hidden class="animation__dots dot2"></div>
                <div hidden class="animation__dots dot3"></div>
            </div>
            <button hidden type="submit" class="main__load-more" id="load-more">Завантажити ще</button>
        </div>
    </main>
    <footer class="footer">
    ...
    let getElementContent = document.getElementById("content");
        ...
        let elementFilmName = evt.target.closest('h2');
        if(elementFilmName !== null){
            window.history.pushState(null, null, `/movie/:${elementFilm.id}`);
            listLi[0].classList.remove('pop-active');
            listLi[1].classList.remove('fav-active');
        }
    ...
    const cardTemplate = ({poster_path, id, title}, isFavMovie) => {
        ...
        result = `<div ${(isFavMovie === true) ? 'isfavorite="true"' : ''} id="${id}" class="card">
            ...
            <h2 class="card__title" id="film-name">${title}</h2>
            </div>`;  
        return result
    }
    //
